...
const newIndex = index.toString().replace(/@vue3-framework\//g, './el-').replace('el-utils', 'utils')
  ...
  if (!noElPrefixFile.test(comp)) {
    if (fs.lstatSync(path.resolve(libDirPath, comp)).isDirectory()) {
      ...
      if (outsideImport.test(imp) || imp.includes('@vue3-framework/')) {
        ...
        }).replace('@vue3-framework/', '../el-').replace('el-utils', 'utils')
        ...
        data.forEach(f => {
          if (!fs.lstatSync(path.resolve(srcPath, f)).isDirectory()) {
            ...
            if (imp.includes('@vue3-framework/')) {
              const newImp = imp.replace(/@vue3-framework\//g, '../../el-')
