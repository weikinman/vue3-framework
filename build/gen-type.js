...
const { novfPrefixFile } = require('./common')
...
fs.copyFileSync(
        ...
        path.resolve(__dirname, '../lib/vue3-framework.d.ts'),
    )
    // index.d.ts
const newIndexPath = path.resolve(__dirname, '../lib/index.d.ts')
fs.copyFileSync(path.resolve(__dirname, '../lib/vue3-framework/index.d.ts'), newIndexPath)
const index = fs.readFileSync(newIndexPath)
const newIndex = index.toString().replace(/@vue3-framework\//g, './vf-').replace('vf-utils', 'utils')
fs.writeFileSync(newIndexPath, newIndex)

// remove ep
fs.rmdirSync(path.resolve(__dirname, '../lib/vue3-framework'), { recursive: true })
...
fs.readdirSync(libDirPath).forEach(comp => {
    if (!novfPrefixFile.test(comp)) {
        if (fs.lstatSync(path.resolve(libDirPath, comp)).isDirectory()) {
            ...
            if (outsideImport.test(imp) || imp.includes('@vue3-framework/')) {
                const newImp = imp.replace(outsideImport, (i, c) => {
                    return i.replace(`../${c}`, `../vf-${c}`)
                }).replace('@vue3-framework/', '../vf-').replace('vf-utils', 'utils')
    ...
    if (fs.existsSync(srcPath)) {
        if (fs.lstatSync(srcPath).isDirectory()) {
            fs.readdir(srcPath, 'utf-8', (err, data) => {
                if (err) return
                    // replace all @vue3-framework in src/*.d.ts
                data.forEach(f => {
                    if (!fs.lstatSync(path.resolve(srcPath, f)).isDirectory()) {
                        const imp = fs.readFileSync(path.resolve(srcPath, f)).toString()
                        if (imp.includes('@vue3-framework/')) {
                            const newImp = imp.replace(/@vue3-framework\//g, '../../vf-')
